...
Rect CalculateRecatangleMBR(Node *n) // Calculating the minimum bounding rectangle function
{
    Rect r_mbr;
    ...
    for (int i = 0; i < n->count; i++) // Traversing the whole node (all the entries of a particular node)
    {
        ...
        else // handling the other cases
        {
            ...
            {
                r_mbr.topRight.x = n->elements[i].mbr.topRight.x; // assign the value if condition satisfies
            }
            else
            {
                r_mbr.topRight.x = r_mbr.topRight.x; // if condition doesn't satisfy ,it remains unchanged
            }
...
{
    Node *node = rtree->root; // initializing  node to root as per step-1 of choose leaf in the research paper

    if (node->is_leaf) // if node is a leaf node then return that according to step 2 of choose leaf in research paper
    {
        return node;
    }
    while (!node->is_leaf) // else part when node is not a leaf nodes
    {
        ...
        float minOverlap = -1.0; // initialize minimum overlap value and index values and storing in float variable as overlap may be in decimal
        int minIndex = -1;

        for (int i = 0; i < node->count; i++) // iterate through each entry of the node and calculate the overlap with the given rectangle
        {
            Rect mbr = CalculateRecatangleMBR(node->elements[i].child);
            float overlap = CalculateOverlapArea(mbr, rect);

            if (minOverlap < 0 || overlap <= minOverlap) // update minimum overlap and index if overlap is smaller or equal to minOverlap
        ...
        node = node->elements[minIndex].child; // moving down to the child node with the smallest overlap
    }
    return node;
}
